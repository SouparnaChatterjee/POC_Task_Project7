...
function integrateArithConstant(model, dev, id) {
  ...
  function help(inConnList, inConnList2, outConnList, sgn, in1, in2) {
    ...
    const newDev = _core.util.cloneDeep(dev);
    if (newDev.bits) {
    ...
    for (const conn of inConnList2) {
      const newConn = _core.util.cloneDeep(conn);
      newConn.to.port = "in";
...
class CircuitModel {
  ...
  addDevice(dev, id) {
    ...
    this._devices[id] = _core.util.cloneDeep(dev);
    this._forward[id] = {};
    this._backward[id] = {};
  }
  getDevice(id) {
  ...
  addConnector(conn, fromConns = []) {
    ...
    this._connectors[id] = _core.util.cloneDeep(conn);
